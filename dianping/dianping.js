...
if (url.includes("/dpmobile") || url.includes("/goodsawardpic")) {
  ...
  const resp = {};
  const traceKey1 = Object.keys(header).find((key) =>
    /^m-(shark-)?traceid$/i.test(key),
  );
  const traceKey2 = Object.keys(header).find((key) =>
    /^(ai|dt|al|u)$/i.test(key),
  );
  ...
  if (headopt1 && !headopt2) {
    ...
    $done({
      bodyBytes: hexStringToArrayBuffer(hexString),
      headers: header,
