/**
 * 范例:https://raw.githubusercontent.com/Sheldontao/Scripts/refs/heads/main/sub-store-template/geo-media-check.js#concurrency=8&media_timeout=5000&retries=2&timeout=8000&youtube=true
...
async function operator(proxies = [], targetPlatform, context) {
  ...
  if (!surge_http_api_enabled && !isLoon && !isSurge)
    throw new Error(
      "请使用 Loon, Surge(ability=http-client-policy) 或 配置 HTTP API",
    );
  ...
  const enabledMediaChecks = MEDIA_PLATFORMS.filter(
    (p) => $arguments[p.key] === "true" || $arguments[p.key] === true,
  );
  const mediaFormat = $arguments.media_format;
  const mediaTimeout = parseFloat(
    $arguments.media_timeout || $arguments.timeout || 5000,
  );

  // 媒体解锁检测 - 常量 (需在 executeAsyncTasks 前初始化)
  const MEDIA_RESULT_SYMBOLS = {
    ok: "✓",
    partial: "◐",
    blocked: "✗",
    error: "?",
  ...
  async function check(proxy) {
      ...
      ? `geo:${url}:${format}:${regex}:${internal}:${JSON.stringify(
          Object.fromEntries(
            Object.entries(proxy).filter(
              ([key]) => !/^(collectionName|subName|id|_.*)$/i.test(key),
            ),
          ),
        )}`
      : undefined;
    ...
    try {
      const node = ProxyUtils.produce(
        [proxy],
        surge_http_api_enabled ? "Surge" : target,
      );
      if (node) {
        // 媒体解锁检测 - 启动异步任务，与 geo 请求并行
        mediaCheckPromise =
          enabledMediaChecks.length > 0
            ? runMediaChecks(node)
            : Promise.resolve("");
        ...
        if (cacheEnabled && cached) {
          if (cached.api) {
            ...
            $.log(
              `[${proxy.name}] api: ${JSON.stringify(cached.api, null, 2)}`,
            );
