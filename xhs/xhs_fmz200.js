...
if (url.includes("/v6/homefeed")) {
  if (obj?.data?.length > 0) {
    ...
    const getCachedRegexes = (key, argValue) => {
      let cachedRegexes = $.getdata(key);
      if (cachedRegexes) {
        try {
          const parsedCached = JSON.parse(cachedRegexes);
          if (Array.isArray(parsedCached) && parsedCached.length > 0) {
            console.log(`Loaded regexes from cache: ${key}`);
            return parsedCached.map(str => new RegExp(str));
          }
        } catch (e) {
          console.error(`Error parsing cached regexes for ${key}: ${e}`);
          // Fallback to argument if cache is corrupted
        }
      }

      const regexStrs = JSON.parse(argValue || "[]");
      const regexes = [];
      if (Array.isArray(regexStrs)) {
        for (const str of regexStrs) {
          try {
            regexes.push(new RegExp(str));
            console.log(`Added regex filter from argument: ${str}`);
          } catch (e) {
            console.error(`Invalid regex provided from argument: ${str}, Error: ${e}`);
          }
        }
        // Cache the valid regex strings
        try {
          $.setdata(JSON.stringify(regexStrs), key);
          console.log(`Cached regexes for ${key}`);
        } catch (e) {
      ...
      if (descRegexes.length > 0 && item?.desc) {
        for (const regex of descRegexes) {
          if (regex.test(item.desc)) {
            console.log(`Filtered out item with desc matching regex: ${item.desc}`);
            return false;
          }
        }
      }
      ...
      if (nicknameRegexes.length > 0 && item?.user?.nickname) {
        for (const regex of nicknameRegexes) {
          if (regex.test(item.user.nickname)) {
            console.log(`Filtered out item with nickname matching regex: ${item.user.nickname}`);
            return false;
          }
        }
      }
