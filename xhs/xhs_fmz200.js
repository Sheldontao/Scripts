...
if (url.includes("/v6/homefeed")) {
  if (obj?.data?.length > 0) {
    ...
    const getCachedRegexes = (key, argValue) => {
      ...
      let currentRegexStrs = argValue; // Prioritize argument value

      if (currentRegexStrs) {
        // Argument is present, compare with cache
        if (currentRegexStrs !== cachedRegexStrs) {
          console.log(`Argument for ${key} differs from cache. Updating cache.`);
          try {
            $.setdata(currentRegexStrs, key);
            cachedRegexStrs = currentRegexStrs; // Update cached value for immediate use
          } catch (e) {
            console.error(`Error caching regexes for ${key}: ${e}`);
          }
        } else {
          console.log(`Argument for ${key} is same as cache. Using argument.`);
        }
      } else if (cachedRegexStrs) {
        // Argument is not present, use cache
        console.log(`Argument for ${key} is empty. Using cached regexes.`);
        currentRegexStrs = cachedRegexStrs;
      } else {
        // Neither argument nor cache has value
        console.log(`No argument or cached regexes for ${key}. Using empty array.`);
        currentRegexStrs = "[]";
      }

      const regexes = [];
      try {
        const parsedRegexStrs = JSON.parse(currentRegexStrs || "[]");
        if (Array.isArray(parsedRegexStrs)) {
          for (const str of parsedRegexStrs) {
            try {
              regexes.push(new RegExp(str));
              console.log(`Added regex filter: ${str}`);
            } catch (e) {
